...
**The Agent's Core Workflow:** Ingest a natural-language business problem → Analyze the accompanying tabular dataset → Formulate an appropriate mathematical optimization approach → Dynamically code and solve the problem using OR libraries (e.g., Google OR-Tools, Gurobi, SciPy) → Translate the raw mathematical results back into clear, actionable business recommendations.
